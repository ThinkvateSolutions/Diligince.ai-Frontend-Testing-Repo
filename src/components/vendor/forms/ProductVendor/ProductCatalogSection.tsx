...
import { 
  ...
  Trash, Edit, ArrowUp, ArrowDown, ChevronsUpDown, FileText,
  Info
} from "lucide-react";
...
const productSchema = z.object({
  ...
  specifications: z.string().min(1, { message: "Specifications are required." }).max(1000, { message: "Specifications must be 1000 characters or less." }),
  price: z.coerce.number({ invalid_type_error: "Price must be a number." }).positive({ message: "Price must be a positive number." }),
  minOrderQuantity: z.coerce.number({ invalid_type_error: "Quantity must be a number." }).int({ message: "Must be a whole number." }).positive({ message: "Minimum order quantity must be positive." }),
  leadTime: z.coerce.number({ invalid_type_error: "Lead time must be a number." }).int({ message: "Must be a whole number." }).positive({ message: "Lead time must be positive." }),
  stock: z.coerce.number({ invalid_type_error: "Stock must be a number." }).int({ message: "Must be a whole number." }).nonnegative({ message: "Stock cannot be negative." }),
...
const VALID_IMAGE_EXTENSIONS_STRING = ".png, .jpeg, .jpg";
const VALID_DOC_EXTENSIONS = [".pdf", ".doc", ".docx", ".ppt", ".pptx", ".xls", ".xlsx", ".csv"];
const VALID_DOC_EXTENSIONS_STRING = ".pdf, .doc, .docx, .ppt, .pptx, .xls, .xlsx, .csv";

// MODIFICATION: Changed drag/hover state to use orange colors
const DragAndDropZone = ({ onFilesSelected, accept, mainText, subText, IconComponent }: { onFilesSelected: (files: File[]) => void; accept: string; mainText: string; subText: string; IconComponent: React.ElementType }) => {
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
      className={`border-2 border-dashed rounded-md p-6 flex flex-col items-center justify-center transition-colors ${isDragging ? "border-orange-500 bg-orange-50" : "border-gray-300"}`}
    >
      <IconComponent className="h-8 w-8 text-muted-foreground mb-2" />
      <p className="text-sm text-muted-foreground text-center mb-2">{mainText}</p>
      <p className="text-xs text-muted-foreground text-center mb-2">{subText}</p>
      <Input ref={inputRef} id="fileUpload" type="file" className="hidden" accept={accept} multiple onChange={handleChange} />
      <Button type="button" variant="outline" onClick={() => inputRef.current?.click()}>Browse Files</Button>
    </div>
  );
};
...
const ProductCatalogSection = () => {
  ...
  const form = useForm<ProductFormValues>({ resolver: zodResolver(productSchema), defaultValues: { name: "", category: "", description: "", specifications: "", price: 0, minOrderQuantity: 0, leadTime: 0, stock: 0, thumbnail: "" } });
  ...
  const handleEditClick = (e: React.MouseEvent, product: Product) => {
    ...
    form.reset({ id: product.id, name: product.name, category: categoryOptions.find(c => c.label === product.category)?.value || "", description: product.description, specifications: product.specifications, price: product.price, minOrderQuantity: product.minOrderQuantity, leadTime: product.leadTime, stock: product.stock, thumbnail: product.thumbnail });
    setImagePreviews(product.thumbnail ? [product.thumbnail] : []);
    setDocumentFiles([]);
    setIsProductDialogOpen(true);
  };
  
  const handleImageFilesChange = (incomingFiles: File[]) => {
    const validFiles = incomingFiles.filter(file => VALID_IMAGE_TYPES.includes(file.type));
    const invalidFiles = incomingFiles.filter(file => !VALID_IMAGE_TYPES.includes(file.type));

    if (invalidFiles.length > 0) {
      toast.error(`Invalid file type(s): ${invalidFiles.map(f => f.name).join(', ')}. Please upload PNG, JPEG, or JPG.`);
    }
    ...
    if (validFiles.length > 0) {
      ...
      validFiles.forEach(file => {
        ...
        reader.onloadend = () => {
          ...
          if (currentPreviews.length === 0 && imagePreviews.length === 0) {
          ...
          if (currentPreviews.length === validFiles.length) {
            setImagePreviews(prev => [...prev, ...currentPreviews]);
          }
        };
  ...
  const handleDocumentFilesChange = (incomingFiles: File[]) => {
    ...
    incomingFiles.forEach(file => {
      ...
      if (VALID_DOC_EXTENSIONS.includes(extension)) {
    ...
    if (validFiles.length > 0) {
      ...
      toast.success(`${validFiles.length} document(s) selected.`);
    }
  };
  ...
  <div className="flex flex-col">
    <div className="flex items-center">
      <CardTitle className="mr-2 text-2xl font-bold text-gray-800">Product Catalog</CardTitle>
      <button
        type="button"
        onClick={() => setShowCatalogInfo(prev => !prev)}
        className="text-muted-foreground hover:text-blue-600 transition"
        aria-label="Toggle catalog information"
      >
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 pt-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><FormField control={form.control} name="name" render={({ field }) => (<FormItem><FormLabel className="text-foreground">Product Name</FormLabel><FormControl><Input placeholder="Enter product name" {...field} /></FormControl><FormMessage /></FormItem>)} /><FormField control={form.control} name="category" render={({ field }) => (<FormItem><FormLabel className="text-foreground">Category</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Select category" /></SelectTrigger></FormControl><SelectContent>{categoryOptions.map((o) => (<SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>))}</SelectContent></Select><FormMessage /></FormItem>)} /></div>
              {/* MODIFICATION: Added character counter */}
              <FormField control={form.control} name="description" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Enter detailed product description..." className="min-h-24" {...field} />
    ...
    <span className="text-xs text-muted-foreground">
      {field.value?.length || 0}/1000
              ...
              <FormField control={form.control} name="specifications" render={({ field }) => (
                <FormItem>
                  <div className="flex justify-between items-center">
                    <FormLabel className="text-foreground">Technical Specifications</FormLabel>
                    
                  </div>
                  <FormControl>
                    <Textarea placeholder="Enter technical specifications, one per line..." className="min-h-24" {...field} />
                  </FormControl>
  ...
  <div className="flex justify-end">
    <span className="text-xs text-muted-foreground">
      {field.value?.length || 0}/1000
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><FormField control={form.control} name="price" render={({ field }) => (<FormItem><FormLabel className="text-foreground">Price (₹)</FormLabel><FormControl><Input type="number" placeholder="e.g., 2500" {...field} /></FormControl><FormMessage /></FormItem>)} /><FormField control={form.control} name="stock" render={({ field }) => (<FormItem><FormLabel className="text-foreground">Stock Availability</FormLabel><FormControl><Input type="number" placeholder="e.g., 25" {...field} /></FormControl><FormMessage /></FormItem>)} /></div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><FormField control={form.control} name="minOrderQuantity" render={({ field }) => (<FormItem><FormLabel className="text-foreground">Minimum Order Quantity</FormLabel><FormControl><Input type="number" placeholder="e.g., 1" {...field} /></FormControl><FormMessage /></FormItem>)} /><FormField control={form.control} name="leadTime" render={({ field }) => (<FormItem><FormLabel className="text-foreground">Lead Time (days)</FormLabel><FormControl><Input type="number" placeholder="e.g., 5" {...field} /></FormControl><FormMessage /></FormItem>)} /></div>
              <Separator />
              <div className="space-y-3"><FormLabel className="text-foreground">Product Images</FormLabel>
                <DragAndDropZone
                  onFilesSelected={handleImageFilesChange}
                  accept={VALID_IMAGE_EXTENSIONS_STRING}
                  mainText="Drag 'n' drop some files here, or click to select files"
                  subText="First image is the thumbnail. Accepts PNG, JPEG, JPG."
                  IconComponent={Image}
                />
                {imagePreviews.length > 0 && (<div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 mt-4">{imagePreviews.map((img, i) => (<div key={i} className="relative rounded-md overflow-hidden border h-24"><img src={img} alt={`Preview ${i}`} className="w-full h-full object-cover" /><Button type="button" variant="destructive" size="icon" className="absolute top-1 right-1 h-6 w-6" onClick={() => setImagePreviews(imagePreviews.filter((_, idx) => i !== idx))}><Trash size={12} /></Button></div>))}</div>)}
              </div>
              <div className="space-y-3"><FormLabel className="text-foreground">Product Documents</FormLabel>
                 <DragAndDropZone
                  onFilesSelected={handleDocumentFilesChange}
                  accept={VALID_DOC_EXTENSIONS_STRING}
                  mainText="Drag 'n' drop documents here, or click to select files"
                  subText="Accepts PDF, DOCX, PPT, CSV, XLSX"
                  IconComponent={FileText}
                />
                {/* MODIFICATION: Styled the document delete button */}
                {documentFiles.length > 0 && (<div className="space-y-2 mt-4">{documentFiles.map((doc, i) => (
                  <div key={i} className="flex items-center justify-between p-3 pl-4 border rounded-md bg-muted/50">
                    <span className="text-sm font-medium truncate pr-2">{doc.name}</span>
                   <button
  type="button"
  aria-label="Remove document"
  onClick={() => setDocumentFiles(documentFiles.filter((_, idx) => i !== idx))}
  className="p-1 text-red-500 hover:text-red-600 transition-colors rounded-md"
>
  <Trash size={16} />
</button>
