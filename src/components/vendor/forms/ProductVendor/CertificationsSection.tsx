...
import * as z from "zod";
import { toast } from "sonner";
import { Plus, Upload, Building, Check, AlertTriangle, Pencil, Trash, Download, MoreVertical, ArrowUpDown, Edit, FileText, Info, ChevronLeft, ChevronRight, X } from "lucide-react";
...
// FIX: Removed certificationType from the validation schema to make the form submittable.
const certificationSchema = z.object({
  id: z.string().optional(),
  certificateName: z.string().min(1, { message: "Certificate name is required" }),
  issuingOrganization: z.string().min(1, { message: "Issuing organization is required" }),
...
type Certification = {
  ...
  issueDate: string;
  expiryDate: string;
  type: string; // This can remain in the type definition if used elsewhere, but it's not part of the form anymore.
  status: "Verified" | "Pending Verification";
  document: string;
  documentUrl: string;
  linkedProducts: string[];
...
const CertificationsSection = () => {
  ...
  const [isLoading, setIsLoading] = useState(false);
  const [certifications, setCertifications] = useState(mockCertifications);
  const [documentFile, setDocumentFile] = useState<{ name: string; url: string; size?: number } | null>(null);
  const [sortField, setSortField] = useState<SortField>("issueDate");
  const [sortDirection, setSortDirection] = useState<SortDirection>("desc");
  const [showInfo, setShowInfo] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  ...
  const sortedCertifications = [...certifications].sort((a, b) => {
    ...
    if (sortField === 'issueDate' || sortField === 'expiryDate') {
        const dateA = a[sortField] === "No Expiry" ? Infinity : new Date(a[sortField]).getTime();
        const dateB = b[sortField] === "No Expiry" ? Infinity : new Date(b[sortField]).getTime();
  ...
  const currentItems = sortedCertifications.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);
  ...
  const handleEditClick = (certification: Certification) => {
    ...
    form.reset({
      ...
      issueDate: certification.issueDate,
      expiryDate: certification.expiryDate === "No Expiry" ? "" : certification.expiryDate,
      linkedProducts: productOptions
        .filter(po => certification.linkedProducts.includes(po.label))
        .map(po => po.value),
      documentFile: undefined,
    });
    setDocumentFile({ name: certification.document, url: certification.documentUrl });
    ...
    setTimeout(() => {
      if (editingCertification) {
        const updatedCert = {
          ...
          issueDate: values.issueDate,
          expiryDate: values.expiryDate || "No Expiry",
          type: linkedProductLabels.length > 0 ? "Product" : "Company", // Infer type
          document: documentFile?.name || editingCertification.document,
          documentUrl: documentFile?.url || editingCertification.documentUrl,
          linkedProducts: linkedProductLabels,
        };
        ...
        const newCertification: Certification = {
          id: (certifications.length + 2).toString(),
          name: values.certificateName,
          organization: values.issuingOrganization,
          issueDate: values.issueDate,
          expiryDate: values.expiryDate || "No Expiry",
  ...
  const handleDocumentUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setDocumentFile({ name: file.name, url: URL.createObjectURL(file), size: file.size });
  ...
  return (
    <div className="space-y-6">
        <Card className="shadow-sm">
            <CardHeader className="flex flex-col md:flex-row md:items-start md:justify-between p-4">
                <div className="flex flex-col">
                    <div className="flex items-center">
                    <CardTitle className="text-2xl text-gray-800 mr-2">Certifications</CardTitle>
                    <button type="button" onClick={() => setShowInfo(!showInfo)} className="text-muted-foreground hover:text-blue-600 transition">
                        <Info className="w-5 h-5" />
                    </button>
                    </div>
                    <div className="min-h-[24px]">
                ...
                <Button onClick={handleAddClick} className="mt-[8px] md:mt-[10px] gap-2 bg-yellow-600 hover:bg-yellow-700">
                    <Plus size={16} />
                    <span>Upload Certification</span>
                </Button>
            </CardHeader>
            <CardContent>
                <div className="space-y-5">
                    <div className="flex gap-2 mb-4 flex-wrap">
                        {(['name', 'organization', 'issueDate', 'expiryDate'] as SortField[]).map(field => (
                            <Button key={field} variant="ghost" size="sm" className="flex items-center gap-1" onClick={() => handleSort(field)}>
                    ...
                    {currentItems.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16 text-center">
                            <Upload className="h-12 w-12 text-muted-foreground mb-4" />
                            <h3 className="text-lg font-medium">No certifications found</h3>
                            <p className="text-sm text-muted-foreground mt-1">Upload your first certification</p>
                        </div>
                    ) : (
                        <>
                            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                                        ...
                                        <CardContent className="p-4 flex flex-col flex-grow">
                                            <div className="flex justify-between items-start mb-3 gap-2">
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center gap-2 truncate">
                                                        <h3 className="font-semibold text-lg truncate">{cert.name}</h3>
                                                        <Badge variant={cert.status === "Verified" ? "default" : "secondary"} className={cert.status === "Verified" ? "bg-green-100 text-green-600" : "bg-orange-100 text-orange-600"}>
                                                            <div className="flex items-center gap-1">
                                                                {cert.status === "Verified" ? <Check className="h-3 w-3" /> : <AlertTriangle className="h-3 w-3" />}
                                                                {cert.status}
                                                            </div>
                                                        </Badge>
                                                    </div>
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end">
                                                        <DropdownMenuItem onClick={() => handleDownload(cert)}><Download className="mr-2 h-4 w-4" /><span>Download</span></DropdownMenuItem>
                                                        <DropdownMenuItem onClick={() => handleEditClick(cert)}><Edit className="mr-2 h-4 w-4" /><span>Edit</span></DropdownMenuItem>
                                                        <DropdownMenuItem onClick={() => handleDeleteCertification(cert.id)} className="text-red-500 focus:text-red-500"><Trash className="mr-2 h-4 w-4" /><span>Delete</span></DropdownMenuItem>
                                                    </DropdownMenuContent>
                                                </DropdownMenu>
                                            </div>
                                            <div className="flex-grow space-y-2 mb-2">
                                                <p className="text-sm text-gray-600">Issued by: <span className="font-medium">{cert.organization}</span></p>
                                                {cert.linkedProducts.length > 0 && (
                                                    <div>
                                                        <p className="text-xs text-muted-foreground">Linked Products</p>
                                                        <div className="flex flex-row flex-wrap items-center gap-2 mt-1">
                                                            {cert.linkedProducts.slice(0, 2).map((product, index) => <Badge key={index} variant="outline">{product}</Badge>)}
                                                            {cert.linkedProducts.length > 2 && <Button variant="link" size="sm" className="h-auto p-0 text-blue-600" onClick={() => handleReadAllProducts(cert.linkedProducts)}>+{cert.linkedProducts.length - 2} More</Button>}
                                                        </div>
                                                    </div>
                                                )}
                                            </div>
                                            {cert.document && <div className="flex items-center gap-2 pb-2 mt-4"><FileText className="h-4 w-4 text-blue-600 flex-shrink-0" /><span className="text-sm text-blue-600 truncate">{cert.document}</span></div>}
                                            <div className="mt-auto pt-3 border-t flex flex-wrap gap-x-4 gap-y-2 text-sm text-gray-600">
                                                <span>Issued: <span className="font-medium">{formatDate(cert.issueDate)}</span></span>
                                                <span>Expiry: <span className="font-medium">{formatDate(cert.expiryDate)}</span></span>
                                            </div>
                                        </CardContent>
                                    </Card>
                                ))}
      ...
      <Dialog open={isAddCertDialogOpen} onOpenChange={setIsAddCertDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>{editingCertification ? "Update the certification details." : "Add a new certification to your profile."}</DialogDescription>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)}>
                <div className="space-y-4 py-2 px-1 pr-3 max-h-[65vh] overflow-y-auto">
                    <FormField control={form.control} name="certificateName" render={({ field }) => (<FormItem><FormLabel>Certification Name</FormLabel><FormControl><Input placeholder="E.g., ISO 9001:2015" {...field} /></FormControl><FormMessage /></FormItem>)} />
                    <FormField control={form.control} name="issuingOrganization" render={({ field }) => (<FormItem><FormLabel>Issuing Organization</FormLabel><FormControl><Input placeholder="E.g., Quality Council of India" {...field} /></FormControl><FormMessage /></FormItem>)} />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <FormField control={form.control} name="expiryDate" render={({ field }) => (
                            <FormItem>
                                <FormLabel>Expiry Date (Optional)</FormLabel>
                                <FormControl>
                                    <Input type="date" {...field} disabled={!issueDate} min={issueDate || undefined} />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                        )} />
                    ...
                    {/* FIX: Removed Certification Type field from the form */}
                    
                    <FormField control={form.control} name="linkedProducts" render={({ field }) => (<FormItem><FormLabel>Link to Products (for product certifications)</FormLabel><FormControl><MultiSelect options={productOptions} selected={field.value || []} onChange={field.onChange} placeholder="Select products to link" className="w-full" /></FormControl><FormMessage /></FormItem>)} />

                    <div className="space-y-2">
                        <FormLabel>Certification Document</FormLabel>
                        <div className="border-2 border-dashed rounded-lg p-6 text-center cursor-pointer hover:border-yellow-500 transition-colors" onClick={() => document.getElementById("certificationDocument")?.click()}>
                            ...
                            <div className="flex flex-col items-center justify-center text-gray-500">
                                <Upload className="text-yellow-500 h-10 w-10 mb-2" /><p className="text-sm">Drag and drop or <span className="font-semibold text-yellow-600">click to browse</span></p><p className="text-xs mt-1">Supports PDF, JPG, PNG up to 5MB</p>
                            </div>
                        </div>
                        {documentFile && (
                            <div className="mt-2 w-full p-2 border rounded-md flex items-center justify-between bg-gray-50">
                                ...
                                <Button type="button" variant="ghost" size="icon" className="h-8 w-8 text-red-500 hover:bg-red-100 flex-shrink-0" onClick={() => { setDocumentFile(null); form.setValue("documentFile", undefined); }}>
                                    <X size={18} />
                                </Button>
                            </div>
                        )}
                ...
                <DialogFooter className="pt-4 border-t mt-4">
                    <Button type="button" variant="outline" onClick={() => setIsAddCertDialogOpen(false)}>Cancel</Button>
                    <Button type="submit" className="bg-yellow-600 hover:bg-yellow-700" disabled={isLoading}>{isLoading ? 'Uploading...' : (editingCertification ? 'Save Changes' : 'Upload')}</Button>
                </DialogFooter>
            </form>
          </Form>
        </DialogContent>
      ...
      <Dialog open={showAllProductsDialog} onOpenChange={setShowAllProductsDialog}>
        <DialogContent className="max-w-md">
            <DialogHeader><DialogTitle>All Linked Products</DialogTitle><DialogDescription>A complete list of products linked to this certification.</DialogDescription></DialogHeader>
            <div className="space-y-2 max-h-60 overflow-y-auto">{currentLinkedProducts.map((product, index) => (<Badge key={index} variant="outline">{product}</Badge>))}</div>
            <DialogFooter><Button onClick={() => setShowAllProductsDialog(false)}>Close</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
