...
const mockBrands: Brand[] = [
  {
    ...
    documents: [{ name: "Midpoint Ellipse Algorithm and 3D Bouncing Animation.docx" }],
  },
  {
    id: "2",
    ...
    documents: [{ name: "SafeGuard_Agreement.pdf" }],
  },
  {
    id: "3",
...
const BrandsPartnersSection = () => {
  ...
  const form = useForm<BrandFormValues>({
    ...
    defaultValues: { brandName: "", relationshipType: "", productCategories: [], partnershipStartDate: "" },
  });
  ...
  useEffect(() => {
    if (isFormOpen) {
      if (editingBrand) {
        ...
        form.reset({ brandName: "", relationshipType: "", productCategories: [], partnershipStartDate: "" });
        setLogoPreview(null);
        setDocumentFiles([]);
      }
    }
  ...
  const handleOpenFormDialog = (brand: Brand | null) => {
  ...
  const handleDownloadDocument = (doc: DocumentFile) => {
    if (!doc.file) {
      toast.info(
        `'${doc.name}' is a placeholder.`, {
          description: "This document doesn't have an uploaded file. Please edit the brand to upload it.",
    ...
    <div className="space-y-6">
      <Card className="shadow-sm">
        <CardHeader className="pb-3">
          {/* MODIFICATION: Removed 'flex-wrap' to prevent layout shift */}
            ...
            <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
              ...
              <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col p-0">
                <DialogHeader className="p-6 pb-4 border-b">
                  ...
                  <DialogDescription>
                    {editingBrand ? "Update details for this brand partnership." : "Add details about a new brand partnership."}
                  </DialogDescription>
                </DialogHeader>
                
                <Form {...form}>
                  <form id="brand-form" onSubmit={form.handleSubmit(onSubmit)} className="flex-grow overflow-y-auto">
                    <div className="p-6 space-y-6">
                      <div className="flex flex-col items-center space-y-3">
                        <div className="h-32 w-32 relative border-2 border-dashed rounded-md flex items-center justify-center overflow-hidden bg-muted/30">
                          {logoPreview ? (
                            <img src={logoPreview} alt="Brand logo preview" className="w-full h-full object-cover" />
                          ) : (
                            <div className="text-center text-muted-foreground"><Upload className="h-8 w-8 mx-auto mb-1" /><span className="text-xs">Brand Logo</span></div>
                          )}
                        </div>
                        <Input id="brandLogo" type="file" className="hidden" accept="image/*" onChange={handleLogoUpload} />
                        <Button type="button" variant="outline" size="sm" onClick={() => document.getElementById("brandLogo")?.click()}>
                          {logoPreview ? "Change Logo" : "Upload Logo"}
                        </Button>
                      </div>
                      
                      <FormField control={form.control} name="brandName" render={({ field }) => ( <FormItem><FormLabel>Brand Name</FormLabel><FormControl><Input placeholder="Enter brand name" {...field} /></FormControl><FormMessage /></FormItem> )} />
                      <FormField control={form.control} name="relationshipType" render={({ field }) => ( <FormItem><FormLabel>Relationship Type</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Select relationship type" /></SelectTrigger></FormControl><SelectContent>{relationshipOptions.map((option) => ( <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>))}</SelectContent></Select><FormMessage /></FormItem> )} />
                      <FormField control={form.control} name="productCategories" render={({ field }) => ( <FormItem><FormLabel>Product Categories</FormLabel><FormControl><MultiSelect options={categoryOptions} selected={field.value} onChange={field.onChange} placeholder="Select product categories..." /></FormControl><FormMessage /></FormItem> )} />
                      <FormField control={form.control} name="partnershipStartDate" render={({ field }) => ( <FormItem><FormLabel>Partnership Start Date</FormLabel><FormControl><div className="relative"><Calendar className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input type="date" className="pl-10" {...field} /></div></FormControl><FormMessage /></FormItem> )} />
                      
                      <div className="space-y-3">
                        <FormLabel>Partnership Documents</FormLabel>
                        <div className="border-2 border-dashed rounded-md p-6 flex flex-col items-center justify-center">
                          <Upload className="h-8 w-8 text-muted-foreground mb-2" />
                          <p className="text-sm text-muted-foreground text-center mb-4">Drop files here or click to browse</p>
                          <Input id="partnershipDocuments" type="file" className="hidden" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" multiple onChange={handleDocumentUpload} />
                          <Button type="button" variant="outline" onClick={() => document.getElementById("partnershipDocuments")?.click()}>Browse Files</Button>
                        </div>
                        {documentFiles.length > 0 && (
                          <div className="w-full space-y-2 mt-4">
                            <p className="text-sm font-medium text-muted-foreground">Uploaded Documents:</p>
                            {documentFiles.map((doc, index) => (
                              <div key={index} className="flex items-center justify-between p-2 pl-3 border rounded-md bg-muted/30">
                                ...
                                <div className="flex items-center">
                                  <TooltipProvider>
                                    <Tooltip>
                                      <TooltipTrigger asChild>
                                        <Button type="button" variant="ghost" size="icon" className="h-7 w-7" onClick={() => handleDownloadDocument(doc)}>
                                  ...
                                  <Button type="button" variant="ghost" size="icon" className="h-7 w-7" onClick={() => setDocumentFiles(documentFiles.filter((_, i) => i !== index))}>
                                    <X size={14} />
                                  </Button>
                                </div>
                              </div>
                ...
                <DialogFooter className="p-4 border-t bg-muted/50">
                  ...
                  <Button type="submit" form="brand-form" className="bg-yellow-600 hover:bg-yellow-700" disabled={isLoading}>
                    {isLoading ? (editingBrand ? "Updating..." : "Adding...") : (editingBrand ? "Save Changes" : "Add Brand")}
                  </Button>
              ...
              <div className={viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" : "flex flex-col gap-4"}>
                {sortedBrands.map((brand) => (
                  <BrandItem 
                    ...
                    brand={brand}
                    viewMode={viewMode}
                    onEdit={() => handleOpenFormDialog(brand)}
...
const BrandItem = ({ brand, viewMode, onEdit, onDelete, onDownloadDocument }: { brand: Brand; viewMode: 'grid' | 'list'; onEdit: () => void; onDelete: () => void; onDownloadDocument: (doc: DocumentFile) => void; }) => {
        ...
        {viewMode === 'grid' && (
          <div className="absolute top-2 right-2 flex gap-1">
            ...
            <AlertDialog open={isAlertOpen} onOpenChange={setIsAlertOpen}>
              <AlertDialogTrigger asChild><Button variant="destructive" size="icon" className="h-8 w-8 opacity-90"><Trash size={14} /></Button></AlertDialogTrigger>
              <AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Are you sure?</AlertDialogTitle><AlertDialogDescription>This will permanently delete '{brand.name}'.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={onDelete} className="bg-red-600 hover:bg-red-700">Delete</AlertDialogAction></AlertDialogFooter></AlertDialogContent>
            </AlertDialog>
          </div>
        )}
      </div>
      <div className={viewMode === 'grid' ? 'p-4' : 'p-4 flex-grow'}>
        ...
        <div className={viewMode === 'grid' ? 'space-y-3' : 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'}>
          <div>
            ...
            <Badge className="mt-1 bg-yellow-100 text-yellow-800 hover:bg-yellow-200 border-yellow-200">{brand.relationshipType}</Badge>
          </div>
          <div className={viewMode === 'list' ? 'lg:col-span-2' : ''}>
            <p className="text-xs text-muted-foreground">Categories</p>
            <div className="flex flex-wrap gap-1 mt-1">{brand.categories.map((c, i) => <Badge key={i} variant="outline" className="bg-gray-100 text-gray-800">{c}</Badge>)}</div>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Partnership Since</p>
            <p className="text-sm font-medium mt-1">{new Date(brand.partnershipStartDate).toLocaleDateString()}</p>
          </div>
          {brand.documents.length > 0 && viewMode === 'grid' && (
            <div>
              <p className="text-xs text-muted-foreground">Documents</p>
              <div className="mt-1 space-y-1">
                {brand.documents.map((doc, i) => (
                  <div key={i} onClick={() => onDownloadDocument(doc)} className="flex items-center gap-1.5 text-sm text-blue-600 underline cursor-pointer hover:text-blue-800">
  ...
  if (viewMode === 'list') {
    return <Card className="overflow-hidden border flex flex-col md:flex-row">{content}</Card>;
  }
